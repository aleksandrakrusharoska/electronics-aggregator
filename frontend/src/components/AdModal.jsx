...
export default function AdModal({ ad, onClose, isSaved, onWishlistToggle, onNavigate }) {
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="grid sm:grid-cols-2 gap-0">
            ...
            <div className="p-5 space-y-4">
              ...
              {/* Removed listing — confirmed via a 404 on re-scrape, not a guess */}
              {currentAd.is_active === false ? (
                <div className="flex items-start gap-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg px-3 py-2 border border-red-100 dark:border-red-800">
                  <svg className="w-3.5 h-3.5 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                  <span>Овој оглас повеќе не е достапен на изворната страница — најверојатно е отстранет или продаден.</span>
                </div>
              ) : (
                /* Not yet re-checked — fall back to the age-based heuristic */
              ...
              >
                {similar.map(s => {
                  ...
                  return (
                    <button
                      key={s.ad_url}
                      onClick={() => { setCurrentAd(s); setImgIdx(0); onNavigate?.(s) }}
