...
const AD_TYPE_ACTIVE_CLS = {
  product: 'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300',
  service: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300',
  wanted:  'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300',
          ...
          {AD_TYPES.map(t => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors ${
                  active
                    ? `${AD_TYPE_ACTIVE_CLS[t.value]} font-medium`
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                }`}
              >
                <span className="flex items-center gap-2">
