...
Системот е поделен на два дела: **`scrapy_project/`**, кој секој ден собира и обработува податоци преку закажани GitHub Actions, и **`backend/`**, кој ги сервира преку API и има свој (одделен) Celery pipeline од агенти.

### Собирање и обработка на податоци — `scrapy_project/agents/`

1. **Classification** — означува дали огласот е `product`, `service` или `wanted` (клучни зборови, без LLM).
...
Одделен, детерминистички Celery синџир (Scraper → De-duplicator → Price Analyst → Alerts), плус Recommender:

- **Scraper** — ги извршува адаптерите за секој портал и запишува нови огласи.
- **De-duplicator** — семантичка дедупликација преку embeddings + pgvector cosine similarity.
- **Price Analyst** — детекција на ценовни аномалии (z-score по кластер/категорија).
- **Recommender** — кластеризација + препораки „сличен производ, подобра цена".
- **Alerts** — засега скелет: логира дека известување треба да излезе, без реално испраќање email/push.
